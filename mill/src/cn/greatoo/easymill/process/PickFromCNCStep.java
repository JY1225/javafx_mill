...
public class PickFromCNCStep extends AbstractStep {
	...
	public void pickFromCNC(Program program, FanucRobot robot, CNCMachine cncMachine,
			WorkPiecePositions workPiecePositions, boolean teached, Controller view)
		...
		Coordinates absoluteOffset = TeachedCoordinatesCalculator.calculateAbsoluteOffset(position,
				getUnloadCNCRelativeTeachedOffset());
		// (90.94, 109.42, 2.45, 0.0, 0.0, 90.0)
		position.offset(absoluteOffset);
		float zSafePlane = clamping.getHeight() + program.getFinishedWorkPiece().getHeight()
				+ clamping.getRelativePosition().getZ();
