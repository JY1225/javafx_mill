...
public class PickFromTableStep extends AbstractStep {
	...
	public void pickFromTable(Program program, FanucRobot robot, CNCMachine cncMachine,
			WorkPiecePositions workPiecePositions, boolean teached, int wIndex, Controller view)
			throws AbstractCommunicationException, RobotActionException, InterruptedException {
		...
		robot.writeServiceGripperSet(program.getUnloadstacker().getGripperHead().getName(),
				program.getUnloadstacker().getGripper(), program.getUnloadstacker().getGripper(), serviceType,
				program.getUnloadstacker().getGripperHead().isGripperInner());
		...
		Coordinates absoluteOffset = TeachedCoordinatesCalculator.calculateAbsoluteOffset(position,
				getUnloadStackerRelativeTeachedOffset());
