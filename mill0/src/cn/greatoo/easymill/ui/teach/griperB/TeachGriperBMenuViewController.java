...
public class TeachGriperBMenuViewController extends Controller{
		...
		if (!gridPane.getChildren().contains(teachPickParent)) {
			try {
				URL location = getClass()
						.getResource("/cn/greatoo/easymill/ui/teach/griperB/TeachPickView.fxml");
	...
	private void openPutView(){
		...
		if (!gridPane.getChildren().contains(teachPutParent)) {
			try {
				URL location = getClass()
						.getResource("/cn/greatoo/easymill/ui/teach/griperB/TeachPutView.fxml");
				fxmlLoader = new FXMLLoader();
				fxmlLoader.setLocation(location);
				fxmlLoader.setBuilderFactory(new JavaFXBuilderFactory());
